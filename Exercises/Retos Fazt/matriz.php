...
class ExampleTest extends TestCase
{
    ...
    public function main()
    {
        ...
        foreach ($this->getRowsData() as $value) {
            array_push($results, $value);
        }
        
        foreach ($this->getColumsData() as $value) {
            array_push($results, $value);
        }
    ...
    public function getRowsData()
    {
        for ($i=0; $i < 17; $i++) {
            for ($j=0; $j < 17; $j++) {
                yield array_product(array_slice($this->board()[$i], $j, 4));
            }
        }
    }

    public function getColumsData()
    {
        for ($i=0; $i < 17; $i++) {
            for ($j=0; $j < 17; $j++) {
                yield array_product(array_column(array_slice($this->board(), $i, 4), $j));
            }
        }
    }
    ...
    public function getBelow(int $row, int $column)
    {
